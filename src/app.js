...
class Superwar {
  ...
  fight = () => {
    ...
    let playerFinalScore = document.getElementById('score');

    playerFinalScore.innerHTML = '1 - 0';
  ...
  calculateScore = () => {
    ...
    var winningPlayer = this.updateWin();
    let score = this.score;
    this.players.map((player) => {
      if (player.type == 'villain') {
        player.win = winningPlayer['villain'];
        score['villain'] += player.wins;
      } else {
        player.win = winningPlayer['hero'];
  ...
  checkWin = () => {
    ...
    if (playerHealth['hero'] > playerHealth['villain']) {
      result = 'hero';
    } else if (playerHealth['hero'] == playerHealth['villain']) {
      result = 'endure';
    } else {
      result = 'villain';
    }
  ...
  updateWin = () => {
    let playerWin = { hero: 0, villain: 0, draw: 0 };
  ...
  totalStrength = (type) => {
    ...
    // Type your code here
    const strength = { hero: 0, villain: 0 };
    for (const player of this.players) {
      if (player.type === 'hero') {
        strength.hero += player.strength;
      } else if (player.type === 'villain') {
